<?php
  ...
  function has_unique_customer_username($username, $current_id="0") {
    global $db;
  ...
  function valid_coupon($cou_id, $pk_date) {
    global $db;
    $lock_sql = "
            SELECT * FROM jxx_coupons
            WHERE cou_id = '" . db_escape($db, $cou_id) . "'
            AND is_available = 1
            AND s_date <= '" . db_escape($db, $pk_date) . "'
            for update
      ";
    $sql = "
            SELECT * FROM jxx_coupons
    ...
    $coupon_count = mysqli_num_rows($result);
    //echo $result;
    //echo $coupon_count;
    mysqli_free_result($result);

    return $coupon_count === 1;
  }
